    ...
    public void onInitialize() {
        CommandRegistrationCallback.EVENT.register((dispatcher, dedicated) -> {
            dispatcher.register(literal("slime").executes(context -> {
                ...
                if (isSlimeChunk) {
                    source.sendFeedback(new LiteralText("Chunk " + xPosition + ", " + zPosition + " is a slime chunk."), false);
                } else {
                    source.sendFeedback(new LiteralText("Chunk " + xPosition + ", " + zPosition + " is NOT a slime chunk."), false);
                }
                return 1;
            }));
        });
